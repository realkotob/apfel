...
- `chatFailure` and `responsesFailure` were identical duplicates of the same HTTP failure builder; they are now one shared `openAIFailure`. Internal only, no change to wire behaviour (#437).
- The `--chat` history file is created `0600` before libedit writes prompts into it, and its parent directories `0700`. libedit's `write_history()` uses `fopen(path, "w")`, which creates the file at `0666 & ~umask` - 0644 under the default umask - so the user's prompts were written into a world-readable file and only chmod'd afterwards. If `write_history` failed, the early return left it at 0644 for good. An existing file that has drifted to a wider mode is now corrected too. History persistence is still opt-in via `APFEL_HISTFILE` (#473).
- A stream retry no longer splices the previous attempt's partial output onto the new attempt's text. `StreamPrintSink` tracked only a character-count high-water mark and assumed a retried generation reproduces the same prefix, so when the retry produced different wording - the normal case at any temperature above 0 - the user saw the first attempt's prefix welded to the tail of the second, with exit code 0 and text that looked plausible. The sink now compares the actual emitted text and signals the discontinuity instead of hiding it (#402).
- Exhausting the MCP re-prompt cap is now reported as an error instead of a successful answer. When the model was still requesting a tool call after 3 rounds, apfel stripped the pending tool-call JSON and returned whatever text was left with `finish_reason: stop`, so the caller received a half-finished answer with no indication that tool work had been abandoned. Stripping the raw JSON is right - it must never leak to the user (#187, #358) - but presenting the remainder as a normal completion was not. Both the CLI and HTTP paths now fail loudly, and the cap lives in one shared constant rather than being written twice (#435).
- `shellOutput` drains the child's stdout before waiting for it to exit, and distinguishes failure from empty output. Reading after `waitUntilExit()` deadlocks once the child's output exceeds the OS pipe buffer - the child blocks in `write(2)` waiting for the parent to read, and the parent will not read until the child exits. The helper also swallowed launch errors, discarded stderr and ignored a non-zero exit status, so all three failure modes were indistinguishable from a successful empty result; it now returns `nil` on failure. Latent rather than live before this - `brew info --json=v2 apfel` is ~4.5 KB against a 64 KiB pipe - but one formula-metadata growth spurt from hanging `apfel --update` (#433).
- A remote (Streamable HTTP) MCP server that rejects `notifications/initialized` now fails the handshake instead of being attached and used anyway. The remote path discarded the error with `try?` while the stdio path propagated it, so the two transports disagreed about whether initialization had to succeed (#432).
- Blocking MCP stdio I/O now runs on a GCD thread instead of `Task.detached`. The code said it ran "off the cooperative thread pool", but `Task.detached` changes context inheritance (priority, task-locals, cancellation), not the executor - a detached task is still scheduled on Swift's cooperative executor, so the synchronous `poll`/`read` loop parked a cooperative worker for the whole exchange while holding a lock and blocking on the subprocess (#431).
- A truncated `/v1/responses` stream now ends with `response.incomplete` instead of `response.completed` carrying a payload that says `"status": "incomplete"`. Both terminal paths computed the correct status and then hardcoded the wrong event name, and `response.incomplete` appeared nowhere in the codebase - so a client switching on the event name, which is what SSE clients do, recorded a truncated answer as a complete one (#412).
- `/v1/responses` returns 400 for malformed input content instead of 500, and rejects image parts instead of silently discarding them. An empty string, a non-string scalar, or a text part with no `text` all reached generation and surfaced as HTTP 500 "Last message has no text content" - caller mistakes reported as server faults - while an `input_image` part was dropped and the request succeeded with 200, so the caller believed the image had been considered. Chat Completions already got both right (`.emptyLastMessageContent` from #233, and `.imageContent`); the Responses surface now matches (#409).
- An unrecognised `role` on any message other than the last is now a 400 instead of silently deleting that turn. `ChatRequestValidator` only checked `messages.last?.role`; earlier messages went to `ContextManager.historyEntry`, which returns `nil` for an unknown role, and the caller's `compactMap` removed them. A typo like `"assistent"` - or the wrong case, `"User"` - deleted conversation history and the answer was generated against a conversation the caller never sent, with HTTP 200 and no indication anywhere in the response. OpenAI returns a 400 here, so SDKs and test suites written against the real API would not catch it either. `developer` is now a recognised instruction role and reaches the model, rather than being accepted and then dropped by the same mechanism (#405).
- `apfel --count-tokens --mcp <server>` now includes the prompt in the reported `total`. `ContextManager.makeSession` deliberately excludes the final prompt from its transcript entries because generation sends it separately, so every other call site wraps the result in `sessionInputEntries(builtEntries:finalPrompt:options:)` to add it back - the `--mcp` branch assigned the raw entries instead. The total was therefore the same no matter how long the prompt was, making both the `fits` verdict and the reported headroom wrong exactly when a user is checking whether a long prompt will fit (#399).
- Repeating `--messages` now follows last-flag-wins like every other repeated flag in the parser. `--messages -` set `messagesFromStdin` and a later `--messages <file>` set `messagesJSON` without clearing it, and `main.swift` checks the stdin flag first - so the file that was read and validated was thrown away, apfel read empty stdin, and it exited 2 with `invalid --messages JSON from stdin` while a perfectly valid conversation file sat unused. The error pointed at stdin, which the user had not asked for on that invocation (#416).
- Unreadable or invalid-UTF-8 piped stdin now fails loudly instead of being silently treated as empty. `readStdinData` mapped any read failure to empty `Data()` and `stdinPromptText` mapped a failed UTF-8 decode to `""`, so `producer | apfel 'summarise this'` returned a confident answer about nothing at all - with exit status 0, so no caller could detect it - and the "piped input was empty" hint actively misled, because the input was not empty. Data loss under a success exit code is the one failure mode a UNIX tool must never have (#397).
- A JSON number outside `Double`'s range in a caller-supplied schema is now rejected with a 400 instead of being silently rewritten to `null`. `AnyCodable` ended with an unconditional `value = nil` fallback, so `{"maximum": 1e999}` became `{"maximum": null}` - the schema apfel applied was not the schema that was sent, the response could violate a constraint the caller believed was in force, and nothing in the request or response indicated the substitution. `RawJSON` backs `response_format.json_schema.schema` and `tools[].function.parameters`, so this reached both. An explicit `null` is still a legitimate value (#455).
- Benchmark cases no longer discard errors with `try?`. Three timed operations swallowed every failure, so an operation that threw on every iteration was still timed and still reported - as a fast, `validated: true` result. A benchmark that measures nothing looked like a benchmark that got faster, which would have hidden exactly the regression the case exists to catch. The root cause was `measure(iterations:)` accepting a non-throwing closure, forcing every call site to discard errors at the boundary; it now takes a throwing closure and rethrows (#436).
- The three #213 permit-leak regression tests now run in CI. `test_stream_permit_release.py` carried a whole-suite `pytest.mark.model`, so the per-PR job's `-m "not model"` filter excluded them, and the job did not name the file anyway - the guard against a remote unauthenticated permit-exhaustion DoS was running nowhere except a full local release. None of the three needs the model: every request fails validation and returns 400 before a `LanguageModelSession` is constructed. They are now marked `serial` (they assert `/health`'s global `active_requests` counter, so they cannot share servers with parallel workers) and run as their own step (#434).
- `/v1/responses` honours the `truncation` field instead of accepting it, silently truncating anyway, and echoing `"disabled"` back regardless. `ResponsesRequest` never decoded the field, the handler hardcoded the newest-first strategy, and the encoder hardcoded the string `"disabled"` into every response - so a caller who explicitly opted out of truncation got silently truncated data and a 200 claiming truncation was off, with no way to detect the loss. `"disabled"` now maps to the strict context strategy and returns 400 on oversized input (matching Chat Completions' `x_context_strategy: strict`, which the two surfaces previously disagreed on), `"auto"` and absent keep newest-first trimming, unknown values are a 400 naming `truncation` in `param`, and the envelope echoes what was actually applied (#391).
- `response_format: json_schema` against a server started with `--mcp` now returns a 400 naming the conflict, instead of being silently ignored while the model answered in free text. `handleChatCompletion` built the `GenerationSchema` up front, but the MCP auto-execute branch returned before the schema-guided generation branch was ever reached. Structured output is documented as a guarantee (#167), so any client that parses the result broke the moment an operator added `--mcp` to the server - with no error, no warning, and nothing visible in the request/response pair (#392).
- Every `system` message after the first is no longer silently discarded. `ContextManager.makeSession` strips all system messages out of the conversation and `buildInstructions` copied only the first one back, so a caller stacking a base system prompt and a task-specific one - which SDKs and agent frameworks routinely do - got a 200 and an answer that ignored instructions they believed were in force, with no warning and no token accounting. All system messages are now joined into the instructions block in order. Also affects Responses `developer` messages and CLI `--messages` combined with `-s` (#390).
- The server sets a 30-second HTTP idle timeout, so a stalled request body can no longer hold a concurrency permit forever. The route handler takes the permit before the body is read and the Hummingbird `Application` was built with no server channel configuration, leaving `HTTP1Channel.Configuration.idleTimeout` at `nil` - no idle, read, or connection timeout anywhere in the stack. `--max-concurrent` connections carrying ~150 bytes each took the server out of service for as long as they were held, and this needed no attacker: a client that died or lost its network mid-upload leaked a permit until the OS reaped the half-open connection. Long generations and streaming are unaffected (#463).
- MCP tool arguments and full tool results are no longer written to the server's stderr without `--debug`. `RequestLog.request_body` and `response_body` are deliberately debug-gated and truncated, but the `events` array carried the same class of data with neither protection, so anything an MCP tool returned - file contents, database rows, API responses, credentials the tool was given - landed in the log on a default `apfel --serve --mcp ...`. Under `brew services` that stderr is a durable, world-readable file (`var/log/apfel.log`, mode 0644). Without `--debug` the event now records only the tool name, ok/error status and result length; with `--debug` arguments and results appear, truncated to 512 characters (#464).
- `--allowed-origins '*'` now fires the same red startup warning as `--no-origin-check`, and the banner reads `disabled (all origins allowed)` instead of the actively misleading `localhost only (..., *)`. The wildcard short-circuits `OriginValidator.isAllowed` to `true` for every origin and forces `Access-Control-Allow-Origin: *`, reaching the identical posture to `--no-origin-check` by a route the #232 warning did not cover. Both the banner and the warning now key off a single `ServerSecurity.originValidationConstrains` predicate (#465).
- A deeply nested JSON value in a caller-supplied schema no longer aborts the server process. `AnyCodable` recursed once per nesting level with no cap; Foundation's own JSON scanner only rejects at ~512 levels, far deeper than the stack of the cooperative-pool thread request bodies are decoded on, so a ~1.3 KB unauthenticated POST to `tools[].function.parameters`, `response_format.json_schema.schema` or `text.format.schema` exhausted the stack and killed the process with SIGBUS, taking every in-flight request with it. Because the crash happened during body decoding, before the handler ran, `--token` did not protect against it. Nesting is now capped at 64 levels and an over-nested body is rejected with a 400 (#462).
- Streaming to a closed stdout pipe (`apfel --stream ... | head -1`) no longer aborts the process with an uncatchable `NSFileHandleOperationException` and a stack trace on the user's terminal. apfel ignores SIGPIPE process-wide (#215), so a vanished reader surfaces as EPIPE, and the legacy non-throwing `FileHandle.write(_:)` converts that into an Objective-C exception Swift cannot catch. Writes now go through `ApfelCore.writeTolerantly`: stdout exits quietly with status 141 (128 + SIGPIPE) the way a UNIX filter does, while a lost diagnostic on stderr is dropped without changing the exit status. The same latent abort in `printStderr`, `printError` and the retry banner is fixed in the same change (#389).
